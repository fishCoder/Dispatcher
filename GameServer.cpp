...
GameSever::GameSever(boost::shared_ptr<tcp::socket> _psocket,MessageList &_msgLst,MapManager &_mm):psocket(_psocket),cpool(sizeof(char)),msgLst(_msgLst),mm(_mm){
    ...
    std::cout << "A NEW GAMESERVER : " << id << std::endl;
    readbuf = new char[1024];
    async_read();
}
/*
void GameSever::operator()(boost::system::error_code ec,std::size_t length){
    ...
    if(!ec){
        reenter(this){
            while(true){
                yield psocket->async_read_some(boost::asio::buffer(bp,2*sizeof(int)),*this);

                bp->body = (char *) cpool.ordered_malloc(bp->len);

                yield psocket->async_read_some(boost::asio::buffer(bp->body,bp->len),*this);
                packege.parse(bp);
                while(packege.hasNext()){
                    ...
                    std::cout << "GameServer msg:" << writer.write(root) << std::endl;
                }
                cpool.free(bp->body);
                cpool.free(bp);
            }
