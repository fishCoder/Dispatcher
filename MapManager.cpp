...
MapManager::MapManager(TaskList & _taskLst):taskLst(_taskLst),rc(param.redis_address,param.redis_port){
    total_used = 0;
}
...
std::pair<unsigned int,std::string> MapManager::get_field_map(int map_type_id,std::string & map_data,std::string & loot_npc_data){
    ...
    for(int i=0 ; i<MAX_GET_TIMES ; i++){
        ...
        rc.sadd(boost::lexical_cast<std::string>(map_type_id),str_map_id);
        use_times = boost::lexical_cast<int>(rc.hget(str_map_id,"use"));
        //std::cout << "[MapManager]: use [" << use_times << "]" << std::endl;
...
void MapManager::useMapKey(std::string str_map_id,int type){
    ...
    if(use_time == param.MAX_USE_NUM){
        ...
        rc.srem(boost::lexical_cast<std::string>(type),str_map_id);
        addDelMapType(type);
    //    std::cout << "[MapManager]:delete a key ["<< type <<"] use [" << use_time << "]" << std::endl;
